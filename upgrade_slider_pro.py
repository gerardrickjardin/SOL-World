...
new_component = r"""const HopeEmojiSlider = ({ currentSelection, onSelect, anchors, currentQ }) => {
    ...
    const getGlowStyles = (zone) => {
        if (zone === 1) {
            return snappedValue >= 1.0 && snappedValue <= 2.0 ? "border-blue-500/30" : "opacity-50 grayscale border-transparent";
        }
        if (zone === 3) {
            return snappedValue >= 2.5 && snappedValue <= 3.5 ? "border-white/20" : "opacity-50 grayscale border-transparent";
        }
        if (zone === 5) {
            return snappedValue >= 4.0 && snappedValue <= 5.0 ? "border-purple-500/30" : "opacity-50 grayscale border-transparent";
        }
        return "";
    };
    ...
    return (
        <div className="w-full flex flex-col gap-10 bg-[#15191e] p-8 md:p-10 rounded-3xl mb-6 shadow-2xl border border-slate-800 relative overflow-hidden">
            ...
            {/* Range Slider */}
            <div className="mt-10 mb-2 px-2 w-full flex flex-col gap-4 relative z-10">
                <div className="relative w-full h-1.5">
                    {/* The native input covers the whole bar and acts as the interaction layer */}
                    <input 
                        type="range" 
                        min="1" 
                        max="5" 
                        ...
                        className="w-full h-1.5 bg-gradient-to-r from-blue-600 via-slate-500 to-purple-600 rounded-lg appearance-none outline-none absolute top-0 left-0 z-30"
                    />
                    
                    {/* The Gliding Visual Thumb */}
                    <div 
                        className={`absolute top-1/2 -translate-y-1/2 -ml-4 w-9 h-9 rounded-full border-[3px] border-white flex items-center justify-center text-white text-[12px] font-bold pointer-events-none z-20 ${isDragging ? '' : 'transition-all duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)]'}`}
                        style={{ 
                            left: `calc(0.5rem + ${((displayValue - 1) / 4) * 100}% - ${((displayValue - 1) / 4) * 1}rem)`,
                            backgroundColor: thumbBgColor,
                            boxShadow: `0 0 20px ${thumbBgColor}`,
                            transform: isDragging ? 'translateY(-50%) scale(1.15)' : 'translateY(-50%) scale(1)'
                        }}
                    >
